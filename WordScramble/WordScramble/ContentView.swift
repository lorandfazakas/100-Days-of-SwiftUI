...
//  WordScramble
//
//  Created by Lorand Fazakas on 2021. 04. 17..
//
...
struct ContentView: View {
    ...
    func addNewWord() {
        let answer = newWord.lowercased().trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
        ...
        guard isTooShort(word: answer) else {
            wordError(title: "Word too short", message: "Your answer must be at least 3 characters long")
            return
        }
        
        guard isTheSameAsRootWord(word: answer) else {
            wordError(title: "Same as the root word", message: "Your answer must be different than the root word")
            return
        }
